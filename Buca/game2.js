...
function drawBall(){
 /*   ctx.beginPath();
    ...
    */
    ...
    ctx.drawImage(PUCK, ball.x, ball.y, ball.width,ball.height);
}

var startingY = 300;
function updateBall(){
    console.log(ball.y);
    if(ball.y <= startingY - 10){
        ball.width -= 1;
        ball.height -= 1;
        startingY = ball.y;
    }
...
let lWall = [lwall1,lwall2,lwall3,lwall4];
...
let rWall = [rwall1,rwall2,rwall3,rwall4];
...
function ballTopCollision(array){
    for(let i = 0; i < array.length; i++){
       ...
       if(ball.y + ball.height > w.y && ball.y < w.y && ball.dy < 0){
            ball.dy = - ball.dy;
       }
    }
}
